...
const ShowForm = () => {
    ...
    useEffect(() => {
        if (formDetails) {
            ...
            if (formDetails.regular === false)
                setFormData((prev) => ({
                    ...prev,
                    subjectCode: formDetails.subjectCode,
                }));
    ...
    const handleDateChange = (e) => {
        ...
        if (dateChanged <= Date.now()) {
            ...
            setErr(
                `Date needs to be less than or equal to ${formatDate(Date.now())} !`,
            );
        }
    };
    ...
    const handlePrevYearEditClick = () => {
        ...
        window.scrollTo({
            top: scrollToPrevFormRef.current.offsetTop,
            behavior: 'smooth',
        });
    };
    ...
    useEffect(() => {
        if (response && !error) {
            ...
            const newFormDetails = forms.map((form) =>
    ...
    const resultValue = () => {
        return resultOptions.filter(
            (option) => option.value === prevYearData.result,
        )[0]?.value;
    };
    ...
    return (
        <div className='flex w-full justify-center border-4'>
            ...
            <div className='my-5 w-[95%] rounded border-2 bg-gray-200 p-3 drop-shadow-xl'>
                <div className='my-2 border-b-2 border-gray-800'>
                    <h1 className='font-jost text-3xl font-bold'>
                        {regular ? 'Regular Form' : 'Re-appear Form'}
                    </h1>
                    <p className='text-sm font-semibold text-gray-600'>
                        Created on: {formatDate(formDetails.createdAt)},{' '}
                        {getTimeFromTimestamp(formDetails.createdAt)}
                    </p>
                    <p className='text-sm font-semibold text-gray-600'>
                        Submitted on: {formatDate(formDetails.updatedAt)},{' '}
                        {getTimeFromTimestamp(formDetails.updatedAt)}
                    </p>
                </div>
                ...
                <form
                    onSubmit={handleSubmit(updatePrevYearData)}
                    ref={scrollToPrevFormRef}
                >
                    <div className='my-8'>
                        <h3 className='font-jost text-lg font-bold text-gray-700'>
                    ...
                    {isEditable && !isPrevYearFormEditing && (
                        <Button
                            type='button'
                            data='Edit Form'
                            onClick={handlePrevYearEditClick}
                        />
                    )}
                    {isPrevYearFormEditing && (
                        <Button type='submit' data='Save Changes' />
                ...
                <div className='my-8'>
                    <div className='relative overflow-x-auto rounded-lg shadow-xl'>
                        <table className='w-full rounded-lg text-left text-sm text-gray-500 rtl:text-right'>
                            ...
                            <tbody>
                                <tr className='border-b bg-white'>
                                    ...
                                    <td
                                        className={`px-6 py-4 ${getApprovedLabel(formDetails.approvedByMentor).color}`}
                                    >
                                        {
                                            getApprovedLabel(
                                                formDetails.approvedByMentor,
                                            ).label
                                        }
                                    </td>
                                </tr>
                                <tr className='border-b bg-white'>
                                    ...
                                    <td
                                        className={`px-6 py-4 ${getApprovedLabel(formDetails.approvedByHOD).color}`}
                                    >
                                        {
                                            getApprovedLabel(
                                                formDetails.approvedByHOD,
                                            ).label
                                        }
                                    </td>
                                </tr>
                                <tr className='bg-white'>
                                    ...
                                    <td
                                        className={`px-6 py-4 ${getApprovedLabel(formDetails.approvedByController).color}`}
                                    >
                                        {
                                            getApprovedLabel(
                                                formDetails.approvedByController,
                                            ).label
                                        }
                                    </td>
                                </tr>
